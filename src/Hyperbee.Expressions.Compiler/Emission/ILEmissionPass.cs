        ...
        Dictionary<int, int>? constantIndices )
    {
        ...
        foreach ( var inst in ir.Instructions )
        {
            switch ( inst.Op )
            {
                ...
                // Fused comparison-branch opcodes (generated by peephole)
                case IROp.BranchEqual:
                    ilg.Emit( OpCodes.Beq, ilLabels[inst.Operand] );
                    break;

                case IROp.BranchNotEqual:
                    ilg.Emit( OpCodes.Bne_Un, ilLabels[inst.Operand] );
                    break;

                case IROp.BranchLessThan:
                    ilg.Emit( OpCodes.Blt, ilLabels[inst.Operand] );
                    break;

                case IROp.BranchLessThanUn:
                    ilg.Emit( OpCodes.Blt_Un, ilLabels[inst.Operand] );
                    break;

                case IROp.BranchGreaterThan:
                    ilg.Emit( OpCodes.Bgt, ilLabels[inst.Operand] );
                    break;

                case IROp.BranchGreaterThanUn:
                    ilg.Emit( OpCodes.Bgt_Un, ilLabels[inst.Operand] );
                    break;

                case IROp.BranchGreaterEqual:
                    ilg.Emit( OpCodes.Bge, ilLabels[inst.Operand] );
                    break;

                case IROp.BranchGreaterEqualUn:
                    ilg.Emit( OpCodes.Bge_Un, ilLabels[inst.Operand] );
                    break;

                case IROp.BranchLessEqual:
                    ilg.Emit( OpCodes.Ble, ilLabels[inst.Operand] );
                    break;

                case IROp.BranchLessEqualUn:
                    ilg.Emit( OpCodes.Ble_Un, ilLabels[inst.Operand] );
                    break;

                case IROp.Switch:
                {
                    var labelIndices = (int[]) ir.Operands[inst.Operand];
                    var switchLabels = new Label[labelIndices.Length];
                    for ( var k = 0; k < labelIndices.Length; k++ )
                        switchLabels[k] = ilLabels[labelIndices[k]];
                    ilg.Emit( OpCodes.Switch, switchLabels );
                    break;
                }
